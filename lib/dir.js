...
function constructIncludedDirectory(data) {
  var defaults, opt, files, splitDirPath, tmp, included,
    src, ext, filename, relfilename, key, work, basepath, fullpath, resolver;
  ...
  if (Array.isArray(data[0])) {
    ...
    resolver = yamlinc.getResolver();
    if(resolver && resolver.readDir) {
        files = resolver.readDir(fullpath);
    }
    if(files == null) {
